    ...
    if( dll != NULL )
    {
        while(dll->head != NULL)
        {
            if(dll->head->next != NULL)
            {
                ...
                node_destroy(dll->head->prev,data_destroy_fn);
            }
            else
            {
                node_destroy(dll->head,data_destroy_fn);
                dll->head = NULL;
            }
...
data_type list_pop_front(List* dll)
{
    ...
    if ( dll != NULL )
    {
        if ( dll-> head != NULL )
        {
            ...
            {
                dll->head = dll->head->next;
                node_destroy(dll->head->prev,NULL);
                dll->head->prev = NULL;
            }

            else
            {
                node_destroy(dll->head,NULL);
                dll->head = NULL;
                dll->last = NULL;
            }
    ...
    if ( dll != NULL )
    {
        if ( dll->last != NULL )
        {
            ...
            if ( dll->last->prev != NULL )
            {
                ...
                node_destroy(dll->last->next,NULL);
                dll->last->next = NULL;
            }

            else
            {
                node_destroy(dll->last,NULL);
                dll->last = NULL;
                dll->head = NULL;
            }
