...
void hash_table_destroy(HashTable* h,void(*destroy_pair)(void* pair))
{
    int size_h_table = hash_table_size(h);
    for(int i = 0;i < size_h_table; i++) //Frees each element of the table
    {
        if( h->bucket[i] != 0) list_destroy(h->bucket[i],destroy_pair);
    }
    free(h->bucket);
    free(h);
}
    ...
    if ( h != NULL && key != NULL )
    {
        ...
        if ( h->bucket[index]->size > 0)
        {
            if ( h->bucket[index]->size > 1 )
            {
                ...
                while(it->current != NULL)
                {
                    ...
                    if ( temp_h_item != NULL )
                    {
                        if ( h->cmp_fn( temp_h_item->key, key) == 0)
                        {
                            if ( parent == NULL ) // BUCKET HEAD 
                            {
                                h->bucket[index]->head = h->bucket[index]->head->next;
                                list_destroy(h->bucket[index],destroy_pair);
                            }
                            else
                            {
                                parent->next = it->current->next;
                                list_destroy(h->bucket[index],destroy_pair);
                            }
                            h->bucket[index]->size--;
                            break;
            ...
            else
            {
                ...
                {
                    list_destroy(h->bucket[index],destroy_pair);
                    h->bucket[index]->head->value = NULL;
                    h->bucket[index]->size--;
                }
            }
        }
